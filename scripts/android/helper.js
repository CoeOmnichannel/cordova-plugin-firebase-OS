...
function addDependencies(buildGradle, context) {
  ...
  var match;
  match = buildGradle.match(/^(\s*)classpath 'com.android.tools.build(.*)/m);
  if(match == null){
    match = buildGradle.match(/^(\s*)classpath "com.android.tools.build(.*)/m);
    ...
    buildGradle = buildGradle.replace(/^(\s*)classpath "com.android.tools.build(.*)/m, modifiedLine);
...
function addRepos(buildGradle) {
  ...
  if(match == null){
    ...
    */
    ...
    if (allProjectsIndex > 0) {
      ...
      secondHalfOfFile = secondHalfOfFile.replace(" repositories repos", modifiedLine);
  ...
  console.log("buildGradle: 2 "+buildGradle);
  return buildGradle;
}
